                ...
                <ul>
                    ...
                    <template v-for="(item, index) in menus">
                        ...
                        <template v-if="!item.pivot.parent_id">
                            <li :key="index">
                                <button type="button" @click="loadComponentOrSubMenu(item)" :class="{
                                ...
                                <ul v-if="openSubmenu === item.id">
                                    <template v-for="(child, childIndex) in menus">
                                        <li v-if="child.pivot.parent_id === item.id" :key="childIndex">
                                            <button type="button" @click="loadComponent(child.component)" :class="{
                                                'bg-gray-100 dark:bg-gray-700':
                                                    component_id === child.component,
                                            }" class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">
                                                <span class="flex-1 ms-6 text-left rtl:text-right whitespace-nowrap">{{
                                                    child.name }}</span>
                                            </button>
                                        </li>
                                    </template>
