...
<template>
  <div>
    <!-- Top Menu Start-->
    <div
      class="px-3 py-3 lg:px-5 lg:pl-3 bg-white dark:bg-gray-800 top_menu"
      :class="{
        dashboard: props.title === 'dashboard' && !sidebarToggler,
        innerPage:
          props.title !== 'dashboard' ||
          (props.title === 'dashboard' && sidebarToggler),
      }"
    >
      <div class="flex items-center justify-between">
        <div
          class="flex items-center justify-start rtl:justify-end select-none"
        >
          <button
          ...
          <a
            :href="'/' + props.title"
            class="topbar_title flex ms-2 md:me-24 select-none"
          >
            <span
              class="self-center text-xl font-semibold sm:text-2xl whitespace-nowrap dark:text-white select-none"
            >
              {{ $t(props.title) }}
            </span>
          </a>
          ...
          <div class="flex items-center ms-3">
            <div class="flex items-center justify-center">
              <label class="toggle mr-3">
                <input
                  type="checkbox"
                  :checked="selected_lang === 'en' ? true : false"
                  @click="ToggleChangeLanguage()"
                />
                <span class="slider"></span>
                <span
                  class="labels"
                  :data-on="selected_lang === 'bn' ? 'bn' : 'BN'"
                  :data-off="selected_lang === 'en' ? 'en' : 'EN'"
                ></span>
              </label>
            </div>
            <div>
