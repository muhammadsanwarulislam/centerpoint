...
async function updateUserByID(id) {
  try {
    ...
    if (index !== -1) {
      user_list.value[index] = response.data.user;
    }
      ...
      <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr class="bg-white">
            ...
            <th scope="col" class="px-4 py-4 text-center">Role</th>
      ...
      <fwb-modal v-if="is_show_modal" @close="closeModal" persistent size="5xl">
        ...
        <template #body>
          <div class="flex flex-wrap pr-6 pl-3">
            <div class="flex items-start pt-4 w-full sm:w-1/2 lg:w-1/2">
              ...
              <div class="flex flex-col items-start w-full">
                <input type="text" v-model="form.username" :class="{
                  'border-red-500': is_validation && form.username === '',
                }"
            ...
            <div class="flex items-start pt-4 w-full sm:w-1/2 lg:w-1/2">
              ...
              <div class="flex flex-col items-start w-full">
                <input type="text" v-model="form.email" :class="{
                  'border-red-500': is_validation && form.email === '',
                }"
                  class="block w-full p-3 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                  placeholder="Enter Email" />
                <span v-if="is_validation && form.email === ''" class="text-red-500">Please enter email
                </span>
            ...
            <div class="flex items-start pt-4 w-full sm:w-1/2 lg:w-1/2">
              ...
              <div class="flex flex-col items-start w-full">
                <select id="activity" v-model="form.role_id" :class="{
                  'border-red-500': is_validation && form.role_id === '',
                }"
                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                  <option selected disabled value="">Select Role</option>
                  <option v-for="(item, index) in role_list" :key="index" :value="item.id">
                    {{ item.name }}
                  </option>
                </select>
                <span v-if="is_validation && form.role_id === ''" class="text-red-500">Please select role
                </span>
              </div>
            </div>
        ...
        <template #footer>
          <div class="flex justify-end gap-3">
            ...
            <fwb-button v-else @click="isEditing ? updateUserByID(user_id) : createUser()" color="green">
