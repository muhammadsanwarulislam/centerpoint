...
async function getMenuByID(id) {
  try {
    const response = await $http(`/menus/${id}`, {
      method: "GET",
    });
...
async function createMenu() {
  try {
    ...
    menu_list.value.unshift(response.data);
    closeModal();
  } catch (error) {
...
async function updateMenuByID(id) {
  try {
    ...
    if (index !== -1) {
      menu_list.value[index] = response.data.menu;
    }
...
async function deleteMenu(id) {
  try {
    ...
    const index = menu_list.value.findIndex((item) => item.id === id);
    if (index !== -1) {
      menu_list.value.splice(index, 1);
    }
...
function reset() {
  is_validation.value = false;
  is_name_exist.value = "";
  is_label_exist.value = "";
  is_ordering_exist.value = "";
}
...
<template>
  <div>
    <h1 class="text-2xl font-bold pb-3">Menu Creations</h1>
    <div class="flex flex-wrap gap-3 items-center justify-between pt-3 pb-5">
      <div class="bg-white xs:w-full sm:w-full md:w-2/5 lg:w-2/5 sm-w-100">
        ...
        <div class="relative">
          ...
          <input v-model="search_data" v-on:keyup.enter="getMenuList"
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <button class="button btn" @click="showModal">+ Add Menu</button>
      </div>
    </div>
    <!-- Table -->
      ...
      <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr class="bg-white">
            ...
            <th scope="col" class="px-4 py-4 text-left">Component</th>
            <th scope="col" class="px-4 py-4 text-center">Ordering</th>
        ...
        <tbody>
          ...
          <tr v-else-if="!is_loading && menu_list.length === 0"
            class="bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 h-80">
            <td colspan="5">
              <div class="flex justify-center">
          ...
          <tr v-else v-for="(item, index) in menu_list" :key="index"
            ...
            <td class="px-4 py-4 text-center text-gray-900">
              ...
              <button class="mr-2 table-btn" @click="deleteMenu(item.id)">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- Pagination -->
    <div v-if="!is_loading && menu_list.length > 0" class="flex justify-center">
      ...
      <fwb-modal v-if="is_show_modal" @close="closeModal" persistent size="5xl">
        ...
        <template #body>
          <div class="flex flex-wrap pr-6 pl-3">
            ...
            <div class="flex items-start pt-4 w-full sm:w-1/2 lg:w-1/2">
              <p class="w-56 pl-3 pr-3">label</p>
              <div class="flex flex-col items-start w-full">
                <input type="text" v-model="form.label" :class="{
                  'border-red-500': is_validation && form.label === '',
                }"
                  class="block w-full p-3 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            ...
            <div class="flex items-start pt-4 w-full sm:w-1/2 lg:w-1/2">
              ...
              <div class="flex flex-col items-start w-full">
                <input type="text" v-model="form.component" :class="{
                  'border-red-500': is_validation && form.component === '',
                }"
                  class="block w-full p-3 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                  placeholder="Enter component" />
                <span v-if="is_validation && form.component === ''" class="text-red-500">Please enter component
                </span>
                <span v-if="is_component_exist" class="text-red-500">
                  {{ is_component_exist }}
                </span>
              </div>
            </div>
            <div class="flex items-start pt-4 w-full sm:w-1/2 lg:w-1/2">
              <label for="activity"
                class="w-56 block pl-3 pr-3 text-sm font-medium text-gray-900 dark:text-white">Ordering</label>
              <div class="flex flex-col items-start w-full">
                <input type="number" v-model="form.ordering" :class="{
                  'border-red-500': is_validation && form.ordering === '',
                }"
                  class="block w-full p-3 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                  placeholder="Enter ordering" />
                <span v-if="is_validation && form.ordering === ''" class="text-red-500">Please enter ordering
                </span>
                <span v-if="is_ordering_exist" class="text-red-500">
                  {{ is_ordering_exist }}
                </span>
              </div>
            </div>
