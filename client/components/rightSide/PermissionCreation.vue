...
async function getPermissionList() {
  try {
    ...
    const response = await $http(
      `/permissions?&option=search&offset=${current_page.value}&limit=${limit.value}&searchData=${search_data.value}`,
      {
        method: "GET",
      }
    );
...
<template>
  <div>
    ...
    <div class="flex flex-wrap gap-3 items-center justify-between pt-3 pb-5">
      <div class="bg-white xs:w-full sm:w-full md:w-2/5 lg:w-2/5 sm-w-100">
        ...
        <div class="relative">
          ...
          <input
            v-model="search_data"
            v-on:keyup.enter="getPermissionList"
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <button class="button btn" @click="showModal">+ Add Permission</button>
      </div>
    </div>
    <!-- Table -->
      ...
      >
        ...
        >
          <tr class="bg-white">
            ...
            <th scope="col" class="px-4 py-4 text-left">Permission</th>
        ...
        <tbody>
          ...
          <tr
            v-else-if="!is_loading && permission_list.length === 0"
            class="bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 h-80"
          >
            <td colspan="3">
              <div class="flex justify-center">
    ...
    <!-- Pagination -->
    <div
      v-if="!is_loading && permission_list.length > 0"
        ...
        <template #footer>
          <div class="flex justify-end gap-3">
            ...
            <fwb-button
              ...
              @click="
                isEditing
                  ? updatePermissionByID(permission_id)
                  : createPermission()
              "
              color="green"
            >
